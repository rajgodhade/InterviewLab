...
export default function AssignStudyMaterial() {
  ...
  const fetchData = async () => {
    // 1. Fetch material
    const { data: matData } = await supabase.from('study_materials').select('*').eq('id', materialId).single();
  ...
  const handleIndividualSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error } = await supabase.from('study_material_assignments').insert(assignments);
      if (error) throw error;
      ...
      const notifications = newStudentIds.map(sid => ({
        ...
        message: `Admin shared a resource with you: ${material?.title}`,
  ...
  const handleBatchSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error } = await supabase.from('study_material_assignments').insert({
        material_id: materialId,
      ...
      if (members && members.length > 0) {
        const notifications = members.map(m => ({
          ...
          title: 'New Study Material Shared',
          message: `Admin shared a resource with your batch: ${material?.title}`,
          link: '/student/study-material'
        }));
        await supabase.from('notifications').insert(notifications);
      }

      showToast('Material assigned to batch successfully!', 'success');
      fetchExistingAssignments();
      setSelectedBatchId('');
    } catch (err: any) {
        ...
        <div>
          ...
          {material && (
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
              ...
              <p style={{ color: 'var(--text-secondary)', fontWeight: 500 }}>{material.title}</p>
            </div>
          )}
        </div>
        <button 
